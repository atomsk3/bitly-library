...
async function main() {
    ...
    for (const bitlink of bitlinks) {
        ...
        fileContent += `Creation Date: ${creationDate}, Bitlink: ${bitlink.link}, Name: ${bitlinkName}, Total Clicks: ${totalClicks}\n`;
    }

    fs.writeFile('all-bitlinks.txt', fileContent, (err) => {
        if (err) {
            console.error('Error writing to file:', err);
        } else {
            console.log('Saved all bitlinks with creation dates, names, and total click data to all-bitlinks.txt');
        }
    });
}
